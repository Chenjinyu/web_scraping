...
import time
import platform
from os.path import basename, exists, dirname, isfile, abspath, getctime, join as pathjoin
...
version 0.0.2
all of files like .txt and .xml will be deleted when they are older than two months(60 days).
...
def delete_older_folders(folder_path, days_del = 60):
    current_date = time.time()
    
    def del_sub_folder(sub_foler_name):
        for root, dirs, sub_files in os.walk(pathjoin(folder_path, sub_foler_name)):
            for d in dirs:
                older_folder_path = pathjoin(root, d)
                if (current_date - getctime(older_folder_path)) > 60 * 60 * 24 * days_del:
                    shutil.rmtree(older_folder_path)
                    LOGGING.info('Delete older folder [%s].' % older_folder_path)
                    
    del_sub_folder('succ')
    del_sub_folder('failed')
...
if __name__ == "__main__":
    ...
    try:
        ...
        if READ_CONFIG.get_item_value('TXT_FOLDER', 'seller_info_check') == 'True':
            ...
            if is_ready_to_generate:
                if prod_txt_is_ready:
                    ...
                    LOGGING.info('Generating txt files to seller folder.')
                    if generate_prod_info_from_db_to_txt(prod_base_info_txt):
                        split_txt_into_subfolder(prod_base_info_txt, seller_info_chk_folder, prod_split_count)
                    LOGGING.info('Generate seller txt files DONE.')
        ...
        # delete old folders.
        delete_older_folders(prod_sub_base_info_folder)
        delete_older_folders(pn_sub_chk_folder)
        delete_older_folders(seller_info_chk_folder)
        delete_older_folders(unstable_sub_part_num_folder)
        
        delete_older_folders(READ_CONFIG.get_item_value('XML_FOLDER', 'price_xml_base_path'))
        delete_older_folders(READ_CONFIG.get_item_value('XML_FOLDER', 'seller_xml_base_path'))
        delete_older_folders(READ_CONFIG.get_item_value('XML_FOLDER', 'new_pn_xml_base_path'))
        delete_older_folders(READ_CONFIG.get_item_value('XML_FOLDER', 'unstable_pn_xml_base_path'))
